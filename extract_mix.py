...
def extract_primates(chr, purpose):
    ...
    with h5py.File(hdf5_filename, 'r') as hdf5_file, h5py.File(primate_filename) as primate_file:
        ...
        primate_feature_data = feature_group.create_dataset('data', (num_samples, len(mix_indices), 1000, 4),
                                                            dtype='uint8')
        
        print('=> copying the features')
        for i in range(num_samples):
            primate_feature_data[i] = feature[i][mix_indices]
            if i % 1000 == 1:
